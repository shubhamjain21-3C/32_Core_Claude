      ...
      <div>
        ...
        <div className="relative">
          ...
          <button type="button" onClick={() => setShowPw(!showPw)}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-[#4a90c4] hover:text-[#7aaecc]">
            {showPw ? <EyeOff size={14} /> : <Eye size={14} />}
          </button>
        </div>
      </div>
      <div className="flex justify-end">
        <a href="#" className="text-[#4a90c4] text-xs hover:text-[#6ab4e8] transition-colors">
          Forgot password?
        </a>
      </div>
      {error && (
